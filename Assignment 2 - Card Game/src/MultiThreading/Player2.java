...
public class Player2 implements Runnable{
	...
	public Player2(String one, String two, String three, String four,
	...
	public void discard() {
		...
		d2 = 0; d3 = 0;
		// If card is wanted
		if (deck.get(0).equals("2") || deck.get(0).equals("3")){
			...
			if(d2 + d3 == 5){
				// If there are more twos than threes
				// Search and discard a three
				if(d2 > d3){
					for(int i = 0; i<hand.size(); i++){
						// If three found
						if (hand.get(i).equals("3")){
							// Print discarded card
							System.out.println("Player 2 discarded a: " + hand.get(i));
				...
				// If there are more threes than twos
				// Search and discard a two
				else{
					for(int i = 0; i<hand.size(); i++){
						// If two found
						if (hand.get(i).equals("2")){
							// Print discarded card
							System.out.println("Player 2 discarded a: " + hand.get(i));
							// Set string discard to discarded card
							setDiscard(hand.get(i));
			...
			// discard an undesired card
				...
				for(int i = 0; i<hand.size(); i++){
					...
					if (!hand.get(i).equals("2") && !hand.get(i).equals("3")){
						// Print discarded card
						System.out.println("Player 2 discarded a: " + hand.get(i));
						// Set string discard to discarded card
						setDiscard(hand.get(i));
	...
	// Card is not desired
		...
		System.out.println("Player 2 discarded a: " + deck.get(0));
		// Set string discard to discarded card
	...
	public void cardDrawn() {
		// Print Drawn card
		System.out.println("Player 2 drew a: " + deck.get(0));
	...
	public void printHand() {
		// Print players hand
		System.out.println("Player 2's hand: "+hand);
	...
	public boolean check(){
		if(hand.get(0).equals(hand.get(1)) && hand.get(0).equals(hand.get(2)) && hand.get(0).equals(hand.get(3))){
			...
			System.out.println("Player 2 exits");
	...
	public void makeFile(){
		try {
			PrintWriter out = new PrintWriter("player2.actions");
